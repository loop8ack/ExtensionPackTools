...
namespace ExtensionManager
{
    ...
    public sealed class VsPackage : AsyncPackage
    {
        protected override async Task InitializeAsync(CancellationToken cancellationToken, IProgress<ServiceProgressData> progress)
        {
            ...
            if (isSolutionLoaded)
            {
                JoinableTaskFactory.RunAsync(() => HandleOpenSolutionAsync(solService, extensionService, cancellationToken)
                ).FileAndForget($"{nameof(ExtensionManager)}/{nameof(HandleOpenSolutionAsync)}"); ;
            }

            // Listen for subsequent solution events
            SolutionEvents.OnAfterOpenSolution += (s, e) => HandleOpenSolutionAsync(solService, extensionService, cancellationToken).ConfigureAwait(false);

            if (await GetServiceAsync(typeof(IMenuCommandService)) is OleMenuCommandService commandService)
            {
                ExportCommand.Initialize(this, commandService, extensionService);
                ExportSolutionCommand.Initialize(this, commandService, extensionService);
                ImportCommand.Initialize(this, commandService, extensionService);
            }
        }
        ...
        private async Task HandleOpenSolutionAsync(IVsSolution solService, IExtensionService es, CancellationToken cancellationToken)
        {
